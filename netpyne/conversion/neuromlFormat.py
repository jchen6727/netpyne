    ...
    for np_pop_id in net.pops:
        ...
        if (
            ...
            for cell_name in list(net.params.cellParams.keys()):
                ...
                if sim.cfg.verbose:
                    print("  -- Checking whether pop %s matches %s: %s" % (np_pop_id, cell_name, cell_param_set0))
                                ...
                                if sim.cfg.verbose:
                                    print(
                                        ...
                                        % (
                                            ...
                                            np_pop.tags[cond] if cond in np_pop.tags else "???",
                                        )
                                    )
                                someMisMatches = True
            ...
            if (
                ...
                for np_sec_name in list(cell_param_set["secs"].keys()):
                    ...
                    if "topol" in np_sec and len(np_sec["topol"]) > 0:
                        ...
                        if not (
                            (np_sec["topol"]["parentX"] == 1.0 or np_sec["topol"]["parentX"] == 0.0)
                    ...
                    if "pt3d" not in np_sec["geom"] or len(np_sec["geom"]["pt3d"]) == 0:
                        ...
                        nml_seg.proximal = neuroml.Point3DWithDiam(x=prox[0], y=prox[1], z=prox[2], diameter=prox[3])
                        dist = np_sec["geom"]["pt3d"][1]
                        nml_seg.distal = neuroml.Point3DWithDiam(x=dist[0], y=dist[1], z=dist[2], diameter=dist[3])
    ...
    for np_pop in list(net.pops.values()):
        ...
        if "cellModel" in np_pop.tags and not np_pop.tags["cellModel"] == "NetStim":
            ...
            pop.properties.append(
                neuroml.Property("color", "%s %s %s" % (myrandom.random(), myrandom.random(), myrandom.random()))
            )
...
try:
    ...
    class NetPyNEBuilder(DefaultNetworkHandler):
            ...
            if isinstance(component_obj, Cell):
                ...
                for seg_grp in cell.morphology.segment_groups:
                    ...
                    for ip in seg_grp.inhomogeneous_parameters:
                        ...
                        ordered_segs, cumulative_lengths, path_prox, path_dist = cell.get_ordered_segments_in_groups(
                            [seg_grp.id], include_cumulative_lengths=True, include_path_lengths=True
                        )
        ...
        def handle_projection(
            ...
            if self.verbose:
                print("A projection: %s (%s) from %s -> %s with syn: %s" % (projName, type, prePop, postPop, synapse))
        ...
        def handle_input_list(self, inputListId, population_id, component, size, input_comp_obj=None):
            ...
            self.popStimLists[inputListId] = {"source": inputListId, "conds": {"pop": population_id}}
